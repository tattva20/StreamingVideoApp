...
class LoadResourcePresenterTests: XCTestCase {
    ...
    func test_didFinishLoadingWithMapperError_displaysLocalizedErrorMessageAndStopsLoading() {
        let (sut, view) = makeSUT(mapper: { [weak self] resource in
            throw self?.anyNSError() ?? NSError(domain: "test", code: 0)
        })

        sut.didFinishLoading(with: "resource")

        XCTAssertEqual(view.messages, [
            .display(errorMessage: "Failed to load resource"),
            .display(isLoading: false)
        ])
    }
    ...
    private func makeSUT(
        ...
        self.trackForMemoryLeaks(view, file: file, line: line)
